...
export function createLuxuryCar(color: number): THREE.Group {
  ...
  const extrudeSettings = {
    depth: 4.5, bevelEnabled: true, bevelThickness: 0.05,
    bevelSize: 0.05, bevelSegments: 3,
  };
  const bodyGeo = new THREE.ExtrudeGeometry(bodyShape, extrudeSettings);
  bodyGeo.center();
  const bodyMat = new THREE.MeshPhongMaterial({
    color: bodyColor, emissive: accentColor, emissiveIntensity: 0.15,
  ...
  const canopyMat = new THREE.MeshPhongMaterial({
    color: accentColor, emissive: accentColor, emissiveIntensity: 0.1,
  ...
  const splitterMat = new THREE.MeshPhongMaterial({
    color: accentColor, emissive: accentColor, emissiveIntensity: 0.3,
  });
  ...
  const wingMat = new THREE.MeshPhongMaterial({
    color: bodyColor, emissive: accentColor, emissiveIntensity: 0.2, shininess: 100,
  });
