...
class OpenRouterManager:
    def __init__(self):
        ...
        self.api_key = None
        self.model = 'xiaomi/mimo-v2-flash'  # Default to MiMo-V2-Flash for agentic vibe-coding
        self.max_tokens = 2000  # Increased for agentic tasks
        self.temperature = 0.6  # Slightly lower for coding precision
        ...
        # FREE Models Only (from OpenRouter scraping)
        # Updated: 2026-05-04
            'sourceful/riverflow-v2-fast-preview': {'input': 0.0, 'output': 0.0, 'type': 'image'},
            'black-forest-labs/flux.2-fast': {'input': 0.0, 'output': 0.06, 'type': 'image'},
            'nvidia/nemotron-nano-12b-vl': {'input': 0.0, 'output': 0.0, 'type': 'multimodal'},
            'xiaomi/mimo-v2-flash': {'input': 0.0, 'output': 0.0, 'type': 'text', 'default': True},
            'openai/gpt-oss-120b': {'input': 0.0, 'output': 0.0, 'type': 'text'},
            'google/gemma-3n-2b-it': {'input': 0.0, 'output': 0.0, 'type': 'multimodal'},
            'meta-llama/llama-3.3-70b-instruct': {'input': 0.0, 'output': 0.0, 'type': 'text'},
            ...
            # MiMo-V2-Flash (Xiaomi) - New agentic model for vibe-coding
    ...
    async def chat(self, messages, model=None):
        ...
        if os.getenv('OFFLINE_MODE', 'false').lower() == 'true':
            import json
            from kernel.utils.paths import get_bundled_path
            fixture_path = get_bundled_path('encapsulated', 'fixtures', 'openrouter_response.json')
            try:
                with open(fixture_path, 'r') as f:
                    mock_response = json.load(f)
                mock_response['offline'] = True
                return mock_response
            except Exception as e:
                return {'error': f'Offline mode fixture error: {str(e)}', 'code': 'OFFLINE_ERROR'}
